...
{
	check_the_buf(p, ((p->len = ft_strlen(p->temp_s)) + p->kostyl3) > (size_t)ft_abs(p->width + p->kostyl1 - p->kostyl2 - p->kostyl3) ?
...
void			funct_for_d(t_p *p)
{
	...
	if (p->prec_found)
		p->zero_on = 0;
	if ((p->temp_s[0] == '-' && p->prec_found) || (p->plus_on && p->temp_s[0] != '-'))
		p->kostyl1 = 1;
	if ((p->plus_on && p->zero_on && p->temp_s[0] != '-') || (p->spaces && p->temp_s[0] != '-'))
		p->kostyl2 = 1;
	if ((p->plus_on && p->temp_s[0] != '-') || (p->temp_s[0] == '-' && p->width && p->width < (int)ft_strlen(p->temp_s)))
		p->kostyl3 = 1;
